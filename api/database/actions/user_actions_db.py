...
def save_user(connection, first_name: str, last_name: str, email: UserEmail, location: str, password: str) -> None:
    with get_cursor(connection) as cursor:
        cursor.execute(INSERT_USER, (first_name, last_name, email, location, password))


def check_email_in_db(connection, email: UserEmail) -> bool:
    with get_cursor(connection) as cursor:
...
def take_hashed_password_for_user(connection, email: UserEmail) -> str:
    with get_cursor(connection) as cursor:
...
def get_user_all_info(connection, email: UserEmail) -> list:
    with get_cursor(connection) as cursor:
